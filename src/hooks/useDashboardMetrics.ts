...
const formatPgTimestamp = (value: dayjs.Dayjs) => value.utc().format('YYYY-MM-DD HH:mm:ss[+00]');
...
export function useDashboardMetrics() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const from = dayjs().utc().subtract(6, 'day').startOf('day');
      const toExclusive = dayjs().utc().add(1, 'day').startOf('day');
      ...
      // seed map with last 7 days to ensure stable chart even when data is missing
  ...
  return { metrics, loading, error, refresh: load, hasData };
}
