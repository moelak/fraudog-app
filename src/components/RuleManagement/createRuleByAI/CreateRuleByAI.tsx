...
import { Box, Stepper, Step, StepLabel, Button, Typography, Alert, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions } from '@mui/material';
import { ruleManagementStore } from '@/components/RuleManagement/RuleManagementStore';
import { useRules } from '@/hooks/useRules';
import { callQuickAnalysis, callDeepAnalysis, estimateTokenCount, validateCSVData, parseCSVData, type TokenEstimation, type StreamingStatus } from '@/services/openaiService';
import { convertOpenAIRuleToDatabase, calculateRuleImpact, type OpenAIRule, type UserContext, type DatabaseRule } from '@/utils/ruleConverter';
...
const steps = ['Select Data', 'Token Analysis', 'Output Rules', 'Review'];
...
const CreateRuleByAI: React.FC = () => {
	...
	const [data, setData] = useState<StepperData>({
		csvFile: null,
		csvContent: '',
		csvData: [] as Record<string, unknown>[], // Fix: Explicitly type as string[][]
		csvHeaders: [],
	...
	const handleNext = async () => {
		if (!canProceed() || data.isProcessing) {
			...
			if (data.isProcessing) {
				...
				switch (activeStep) {
					case 0:
						errorMsg = data.piiDetected && !data.piiAcknowledged ? 'Potential PII detected. Accept the warning to proceed.' : 'Please upload a CSV file to proceed.';
						break;
					case 1:
		...
		const performAnalysis = async () => {
			try {
				...
				if (data.analysisType === 'quick') {
					...
					result = await callDeepAnalysis(data.csvData, data.fileName, data.userInstructions, (update) => {
	...
	const saveRule = async () => {
		...
		try {
			...
			// ✅ 1. Create the rule in Supabase

			const createdRule = await createRule(ruleToSave);
			const createdRuleId = createdRule?.id;

			// ✅ 2. Log the event AFTER successful creation
			await logEvent(
				ruleManagementStore.organizationId ?? 'unknown',
				'rule.created',
				'rule', // subject_type
				createdRuleId ?? 'unknown', // subject_id
				{
					rule: ruleToSave,
				}
			);
	...
	return (
		...
		>
			<Box sx={{ width: '100%', maxWidth: 800 }}>
				...
				{/* Step content */}
				<Box sx={{ mt: 4, minHeight: 400 }}>{getStepContent()}</Box>
