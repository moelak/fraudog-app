...
type WidgetId = (typeof EXECUTIVE_WIDGETS)[number] | (typeof OPERATIONS_WIDGETS)[number] | 'shared-loss-trend' | 'shared-friction-stack';
...
const LoadingState = () => <div className='flex h-full items-center justify-center text-sm text-gray-500'>Loading…</div>;

const EmptyState = ({ message }: { message: string }) => <div className='flex h-full items-center justify-center px-6 text-center text-sm text-gray-400'>{message}</div>;
				...
				<div className='relative w-full h-[320px] sm:h-[360px] lg:h-[420px] flex-1 basis-0 min-w-0'>
					{executive.isUsingSampleData && (
						<div className='absolute top-2 right-2 z-10'>
							<span className='inline-flex items-center px-2 py-1 text-xs font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded'>Sample Data</span>
						</div>
					)}
			...
			return (
				<div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
					<div className='rounded-xl border border-gray-100 bg-slate-50 p-4'>
						...
						{averageWeeklyFriction && <p className='mt-2 text-xs text-slate-500'>Avg weekly impact: {formatter.format(Math.round(averageWeeklyFriction))}</p>}
					</div>
					<div className='space-y-3 rounded-xl border border-gray-100 bg-white p-4'>
							...
							{orderedDecisions.map(([decision, value]) => (
								<li key={decision} className='flex items-center justify-between text-sm'>
									<span className='flex items-center gap-2 text-slate-600 capitalize'>
										<span className='inline-flex h-2.5 w-2.5 rounded-full' style={{ backgroundColor: decisionColors[decision] ?? '#cbd5f5' }} />
										{decision}
									</span>
						...
						{topDecision && (
							<p className='text-xs text-slate-500'>
								Largest share: {topDecision.name} ({formatter.format(topDecision.value)})
							</p>
						)}
					</div>
				</div>
		...
		render: ({ operations }) => {
			...
			const cards = [
				...
				},
				{
					...
					value: operations.ruleMonitoring.avgEffectiveness !== null ? `${operations.ruleMonitoring.avgEffectiveness.toFixed(1)}%` : 'N/A',
						...
						<tbody className='divide-y divide-gray-100 bg-white text-sm text-gray-600'>
							{operations.rulesTable.map((rule) => (
								<tr key={rule.id} className='hover:bg-slate-50'>
									...
									<td className='px-4 py-3 text-right font-medium text-slate-900'>{rule.effectiveness !== null ? `${rule.effectiveness.toFixed(1)}%` : 'N/A'}</td>
								</tr>
							))}
						</tbody>
					</table>
					...
					{operations.alerts.slice(0, 5).map((alert) => (
						<article key={alert.id} className='rounded-xl border border-gray-100 bg-white p-4 shadow-sm'>
							<div className='flex items-start justify-between gap-3'>
								...
								<span
									className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold uppercase tracking-wide ${severityBadgeClasses(
										alert.severity
									)}`}
								>
									{alert.severity}
...
const Overview = observer(() => {
	const { rules, activeRules, needsAttentionRules, loading: rulesLoading, fetchRules, searchByDateRange } = useRules();

	const { user } = useAuth();
	const { dateRange, setDateRange: setStoreDateRange, dashboardMetrics, executiveMetrics, operationsMetrics, refreshAll } = overviewStore;
	...
	const dateRangeKey = useMemo(() => `${dateRange.from?.valueOf() ?? 'null'}-${dateRange.to?.valueOf() ?? 'null'}`, [dateRange.from, dateRange.to]);
	...
	const monitoringAlerts = monitoringStore.filteredLogs.slice();
	...
	const executiveContext: ExecutiveWidgetContext = useMemo(() => {
		...
		const lossBreakdownChart: ChartData<'doughnut'> | null =
			totalFraud + totalNonFraud > 0
				? {
		...
		const frictionStackedChart: ChartData<'line'> | null = sortedWeeksDecision.length
			? {
					...
					datasets: decisions.map((decision, index) => ({
						label: decision,
						data: sortedWeeksDecision.map((week) => decisionRows.find((row) => row.week === week && row.decision === decision)?.percentage ?? 0),
		...
		const topDecision = nonAllowEntries.sort((a, b) => b[1] - a[1]).map(([name, value]) => ({ name, value }))[0];
	...
	const operationsContext: OperationsWidgetContext = useMemo(() => {
		...
		const ticketTrendChart: ChartData<'line'> | null = weeks.length
			? {
					...
					datasets: decisions.map((decision, index) => ({
						label: decision,
						data: weeks.map((week) => decisionRows.find((row) => row.week === week && row.decision === decision)?.percentage ?? 0),
						borderColor: decisionColors[decision] ?? defaultPalette[index % defaultPalette.length],
						backgroundColor: decisionAreaFill(decision),
						fill: true,
						tension: 0.35,
	...
	const handleToggleWidget = (widgetId: WidgetId) => {
		...
		setViews((prev) =>
			prev.map((view) => {
				...
				const nextWidgets = exists ? view.widgets.filter((id) => id !== widgetId) : [...view.widgets, widgetId];
	...
	const handleRemoveWidget = (widgetId: WidgetId) => {
		if (!isCustomView) return;
		setViews((prev) => prev.map((view) => (view.id === currentView.id ? { ...view, widgets: view.widgets.filter((id) => id !== widgetId) } : view)));
		setLayoutsByView((prev) => ({
			...prev,
