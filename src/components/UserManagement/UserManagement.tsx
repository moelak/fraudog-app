...
import { UserIcon, EnvelopeIcon, CalendarIcon, MagnifyingGlassIcon, TrashIcon, PencilIcon } from '@heroicons/react/24/outline';
		...
		<div className='space-y-6'>
			...
			{error && (
				<div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4'>
					<p className='text-yellow-700'>Note: User management requires admin privileges. Showing demo data.</p>
				</div>
			)}
			...
			<div className='bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden'>
				...
				{filteredUsers.length === 0 ? (
					<div className='text-center py-12'>
						...
						<h3 className='mt-2 text-sm font-medium text-gray-900'>{searchQuery ? 'No users found' : 'No users'}</h3>
						<p className='mt-1 text-sm text-gray-500'>{searchQuery ? 'Try adjusting your search terms.' : 'Users will appear here when they sign up.'}</p>
					</div>
				) : (
							...
							<tbody className='bg-white divide-y divide-gray-200'>
								{filteredUsers.map((user) => (
									<tr key={user.id} className='hover:bg-gray-50 transition-colors'>
										...
										<td className='px-6 py-4 whitespace-nowrap'>
											<span className='text-sm font-mono text-gray-600'>{user.id.substring(0, 12)}...</span>
										</td>
										<td className='px-6 py-4 whitespace-nowrap'>
											<div className='flex items-center'>
