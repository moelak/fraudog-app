					...
					<div className='space-y-3'>
						...
						<div className='space-y-2 text-sm'>
							...
							<div className='flex items-center space-x-2'>
								...
								<span className='text-gray-600'>Joined: {user?.created_at ? new Date(user.created_at).toLocaleDateString() : 'N/A'}</span>
							</div>
						</div>
					</div>
				</div>
				...
				<div className='space-y-6'>
					...
					<div className='border-b pb-6'>
						...
						<div className='space-y-4'>
							...
							<div className='flex items-center justify-between'>
								<div>
									<label className='text-sm font-medium text-gray-700'>Enable AI Chat Assistant</label>
									<p className='text-sm w-80 text-gray-500'>Enabling the AI Chat Assistant will display it at the bottom right of the page.</p>
								</div>
								<button
									onClick={() => settingsStore.toggleSetting('enableChatAssistant')}
									className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
										settingsStore.settings.enableChatAssistant ? 'bg-blue-600' : 'bg-gray-200'
									}`}
								>
									<span
										className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
											settingsStore.settings.enableChatAssistant ? 'translate-x-6' : 'translate-x-1'
										}`}
									/>
								</button>
							</div>
					...
					<div>
						...
						<div className='space-y-4'>
							<div>
								<label className='block text-sm font-medium text-gray-700 mb-2'>Risk Score Threshold: {settingsStore.settings.alertThreshold}</label>
								<input
									type='range'
									min='1'
									max='100'
